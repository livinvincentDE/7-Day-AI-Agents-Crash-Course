...
Deploy to HF : set GROQ_API_KEY in Space secrets
"""
...
# ── Secrets: HF Spaces injects env vars; dotenv is a no-op there ──────────────
try:
...
# ── Guard: warn if API key missing ────────────────────────────────────────────
if not os.getenv("GROQ_API_KEY"):
    st.error(
        "⚠️ **GROQ_API_KEY** is not set.\n\n"
        "- **Local**: add it to a `.env` file.\n"
        "- **Hugging Face**: go to *Settings → Repository secrets* and add `GROQ_API_KEY`.",
        icon="🔑",
    )
    st.stop()
...
if prompt := st.chat_input("Ask your question..."):
    ...
    # Optional logging — silently skip if evaluation module unavailable
    if result:
        try:
            from evaluation import log_interaction_to_file
            log_path = log_interaction_to_file(agent, result.all_messages(), source="user")
            if log_path:
